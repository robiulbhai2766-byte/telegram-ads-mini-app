...
export default function AdminPage() {
  ...
  async function addAd(e) {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        setMessage(data.message || "Unable to add ad.");
        return;
      }
  ...
  async function toggleAd(ad) {
    try {
      ...
      if (!res.ok || !data.success) {
        setMessage(data.message || "Unable to update ad.");
        return;
      }
      ...
      setMessage("Unable to update advertisement.");
    }
  }
  ...
  async function deleteAd(id) {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        setMessage(data.message || "Unable to delete ad.");
        return;
      }
      ...
      setMessage("Unable to delete advertisement.");
    }
  }
            ...
            {ads.map((ad) => (
              <div key={ad.id} style={styles.adItem}>
                <div>
                  ...
                  <p>
                    💰 Reward: $
                    {Number(ad.reward).toFixed(3)}
                  </p>

                  <p>
                    📊 Daily limit: {ad.daily_limit}
                  </p>
